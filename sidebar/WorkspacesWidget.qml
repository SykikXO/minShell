      ...
      Repeater {
        ...
        Rectangle {
          ...
          Text {
            ...
            font.pixelSize: 18
            color: area.containsMouse ? Theme.c(15) : Theme.textPrimary
            text: btn.isCurrent ? "radio_button_checked" : (btn.isActive ? "adjust" : "fiber_manual_record")
            opacity: area.containsMouse ? 1.0 : (btn.isCurrent ? 1.0 : (btn.isActive ? 0.5 : 0.3))
          }

          function switchWorkspace() {
            for (let i = 0; i < Hyprland.workspaces.count; i++) {
              let ws = Hyprland.workspaces.get(i);
              if (ws.id === btn.workspaceId) {
                ws.activate();
                return;
              }
            }
            Hyprland.dispatch("hl.dsp.focus({workspace = " + btn.workspaceId+"})");
          }
